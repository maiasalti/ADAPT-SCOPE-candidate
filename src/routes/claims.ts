...
import { getPolicyById, updateClaimStatus, withdrawClaim } from '../services/policyDb';
...
claimsRouter.post(
  ...
  (req, res) => {
    ...
    if (result.outcome === 'conflict') {
      res.status(409).json({ error: `Claim cannot be withdrawn from status: ${result.status}` });
      return;
    }

    console.log('Claim withdrawn', redact({ claimId: result.claim.id, status: result.claim.status }));
